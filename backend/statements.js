...
export async function handleGetStatements(request, env, perfumeId) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch statements' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
...
export async function handleCreateStatement(request, env, perfumeId) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to create statement' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
...
export async function handleVoteStatement(request, env, statementId) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to vote on statement' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
