...
export async function handleGetPerfumers(request, env) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch perfumers' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
...
export async function handleGetPerfumer(request, env, perfumerId) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch perfumer' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
...
export async function handleGetPerfumeNoses(request, env, perfumeId) {
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch perfumers for perfume' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}

export async function handleLinkPerfumer(request, env) {
  try {
    // Require auth for linking perfumers
    ...
    return new Response(JSON.stringify({ error: 'Failed to link perfumer' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}
